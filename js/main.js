...
(function () {
    ...
    function onScroll() {
        ...
        if (bgImg) bgImg.style.transform = 'translateY(' + offset + ')';
    }
...
// ===== RSVP FORM =====
const APPS_SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbwVRaj-00knrGgXPprCNnyVG9wX2VGJaZFq0rTS8kr8kEl2V-C64YKUrUyJOPLmEzRNLA/exec';

const rsvpForm    = document.getElementById('rsvpForm');
const rsvpSuccess = document.getElementById('rsvpSuccess');

rsvpForm.querySelector('select').addEventListener('change', function() {
    const guestsInput = rsvpForm.querySelector('input[type="number"]');
    if (this.value === 'no') {
        guestsInput.value = 0;
    } else if (this.value === 'yes' && guestsInput.value === '0') {
        guestsInput.value = '';
    }
});
...
rsvpForm.addEventListener('submit', (e) => {
    ...
    const selectEl = rsvpForm.querySelector('select');
    const guestsInput = rsvpForm.querySelector('input[type="number"]');
    const textArea = rsvpForm.querySelector('textarea');

    const name = nameInput ? nameInput.value.trim() : '';
    const attendance = selectEl ? selectEl.value : '';
    const guests = guestsInput ? guestsInput.value || 1 : 1;
    const text = textArea ? textArea.value.trim() : '';

    if (name && text) addWishItem(name, text, false);

    const btn = document.getElementById('rsvpSubmit');
    const _l = document.documentElement.lang;
    btn.textContent = _l === 'en' ? 'Sending...' : 'Mengirim...';
    btn.disabled = true;
    ...
    setTimeout(() => {
        btn.textContent = document.documentElement.lang === 'en' ? 'Confirmed ✓' : 'Dikonfirmasi ✓';
...
function loadWishesFromSheet() {
    const _l0 = document.documentElement.lang;
    wishesList.innerHTML = '<p class="wishes-loading" data-i18n="wishes-loading">' + (_l0 === 'en' ? 'Loading wishes...' : 'Memuat ucapan...') + '</p>';
    fetch(APPS_SCRIPT_URL, { redirect: 'follow' })
        .then(function(r) { return r.json(); })
        .then(function(data) {
            wishesList.innerHTML = '';
            if (data.status === 'success' && data.wishes && data.wishes.length > 0) {
                data.wishes.forEach(function(w) {
                    addWishItem(w.name, w.message, true, w.timestamp);
                });
            } else {
                wishesList.innerHTML = '<p class="wishes-empty" data-i18n="wishes-empty">' + (document.documentElement.lang === 'en' ? 'No wishes yet. Be the first!' : 'Belum ada ucapan. Jadilah yang pertama!') + '</p>';
            }
        })
        .catch(function() {
            wishesList.innerHTML = '<p class="wishes-error" data-i18n="wishes-error">' + (document.documentElement.lang === 'en' ? 'Failed to load wishes.' : 'Gagal memuat ucapan.') + '</p>';
        });
}

// ===== GUESTBOOK / WISHES =====
function addWishItem(name, text, append, timestamp) {
...
(function () {
    var t = {
        id: {
            ...
            'guest-yth': 'KEPADA YTH:',
            'guest-honorific': 'Bapak/Ibu/Saudara/i',
            'guest-place': 'di Tempat',
            'guest-apology': 'Mohon maaf apabila terdapat kesalahan penulisan nama dan gelar',
            ...
            'title-mr': 'Bapak',
            'title-mrs': 'Ibu',
            'verse-main': '"Demikianlah tinggal ketiga hal ini, yaitu<br>iman, pengharapan dan kasih,<br>dan yang paling besar di antaranya ialah kasih."',
            ...
            'tl-2017b-p': 'Pertemuan pertama di sebuah Bazar PMKO menjadi awal, di mana tawa terasa ringan dan percakapan mengalir tanpa jeda. Sejak saat itu, kami berjalan sebagai sahabat — saling menguatkan, saling menjaga, dalam langkah yang tenang dan sederhana.',
            ...
            'wishes-error': 'Gagal memuat ucapan.'
        },
        en: {
            'open-btn': 'Open Invitation',
            'guest-yth': 'DEAR:',
            'guest-honorific': 'Mr./Mrs./Ms.',
            'guest-place': 'At Your Place',
            'guest-apology': 'We sincerely apologize for any errors in the spelling of names or titles',
            ...
            'title-mr': 'Mr.',
            'title-mrs': 'Mrs.',
            'verse-main': '"And now these three remain:<br>faith, hope and love,<br>but the greatest of these is love."',
            ...
            'tl-2017b-p': "Our first meeting at a PMKO Bazaar marked the beginning of a simple yet meaningful story. There, laughter came easily, and conversations flowed effortlessly. From that moment on, we walked together as friends — encouraging one another, looking out for each other, and growing side by side through life's journey with calm and purpose.",
            'tl-2021-h': 'Reunited',
            ...
            'wishes-error': 'Failed to load wishes.'
        }
    };
